...
package "treeco.dialects.crown" {
  class TreeEnsembleOp <<IRDLOperation>> {
    +buffer_in: MemRefType
    +buffer_out: MemRefType
    +ensemble: TreeEnsembleAttr
  }

  TreeEnsembleOp ..> treeco.dialects.treeco.TreeEnsembleAttr : uses
}
...
package "treeco.dialects.onnxml" {
  class TreeEnsembleClassifier <<IRDLOperation>> {
    +buffer_in: MemRefType
    +buffer_out: MemRefType
    +classlabels_int64s?: ArrayAttr[IntegerAttr]
    +classlabels_strings?: ArrayAttr[StringAttr]
  ...
  class TreeEnsembleRegressor <<IRDLOperation>> {
    +buffer_in: MemRefType
    +buffer_out: MemRefType (result)
    +base_values?: ArrayAttr[FloatAttr]
    +n_targets: IntegerAttr
    +nodes_*: ArrayAttr[...]
    +post_transform: StringAttr
    +target_*: ArrayAttr[...]
  }
...
package "treeco.dialects.extended.ml_program" {
...
package "treeco.dialects.extended.tensor" {
  class ExtractOp <<IRDLOperation>> {
    +tensor: TensorType
    +indices: var IndexType
    +result: element type of tensor
  }
...
package "treeco.dialects.extended.bufferization" {
  class MaterializeInDestinationOp <<IRDLOperation>> {
    +source: TensorType|MemRefType
    +dest: TensorType|MemRefType
    +restrict?: UnitAttr
    +writable?: UnitAttr
    +res*: var results
  }
...
package "treeco.frontend" {
  class Parser {
    +file: Path
    +nodes: list
    +parseModel(): Mapping
    +_parseTreeEnsembleClassifier(node, graph): Mapping
    +_parseTreeEnsembleRegressor(node, graph): Mapping
  }
...
Parser ..> ParseWarning
Parser ..> ParseError
Parser ..> onnx.NodeProto : uses (external)
Parser ..> onnx.GraphProto : uses (external)
...
package "treeco" {
  ...
  class parse_ensemble <<function>> { }
  class generate_ir <<function>> { }
  class crown_transform <<function>> { }
  class trunk_transform <<function>> { }
  class root_transform <<function>> { }
}
